...
class Reservation {
	...
	public function find($reservationId, $userId = null){
		...
		$data = $this ->_db->action('SELECT user.user_id, user.email_id, user.firstname, user.lastname, user.address_line_one, user.address_line_two, user.city,user.country, user.contact_no, room_reservation.reservation_id, room_reservation.requestTimestamp, room_reservation.adults, room_reservation.children, room_reservation.check_in, room_reservation.check_out, DATEDIFF(room_reservation.check_out, room_reservation.check_in) AS nightstays, room_reservation.rate,room_reservation.adults_actual, room_reservation.children_actual, room_reservation.check_in_actual, room_reservation.check_out_actual, room_reservation.total_amount,room_reservation.deposit_amount, room_reservation.additional_amount, room_reservation.balance_amount, room_reservation.cancelled, room_reservation.room_id, room_reservation.room_no, room.room_name, room_allocation.door_no' , 'room_reservation INNER JOIN user ON (room_reservation.user_id = user.user_id) RIGHT JOIN room_allocation ON (room_allocation.room_id = room_reservation.room_id AND room_allocation.room_no = room_reservation.room_no) INNER JOIN room ON (room_reservation.room_id = room.room_id)', $where);
